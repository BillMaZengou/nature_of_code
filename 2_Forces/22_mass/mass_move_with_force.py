...
class Mover(object):
    ...
    def __init__(self):
        ...
        self.loc = np.array([np.random.uniform(10, WIDTH-10), np.random.uniform(10, HEIGHT-10)])
        self.vel = np.zeros(2)
        self.acc = np.zeros(2)
    ...
    def checkEdge(self):
        if (self.loc[0]+self.dis) > WIDTH:
            self.loc[0] = self.loc[0] - self.dis
            self.vel[0] = -self.vel[0]
        elif (self.loc[0]-self.dis) < 0:
            self.loc[0] = self.loc[0] + self.dis
            self.vel[0] = -self.vel[0]
